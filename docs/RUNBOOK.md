...
5. Pin its hash for the cross-language test. Add `"<id>": "<hash>"` to
   ...
   ```
6. If it belongs to a cup, add `"cup"` and `"difficulty"` to its `index.json` entry by hand, and
   list it in [race/courses/CUPS.md](../race/courses/CUPS.md).
...
**Verify:** `python -m pytest race/test/test_add_course.py -q`, then the full
...
- **`add_course.py` rewrites the `index.json` entry** as `{id, name, file}`, which drops `cup` and
  `difficulty`. Re-add them after re-importing an existing course.
- Any field other than `id`/`name`/`version`/`aircraftId`/`startType`/`itemBoxes`/`gates` is
  silently dropped by both the client and `add_course.py`. Keep course notes in CUPS.md.
...
**Sources (`--source`):**
- `usgs` (default): USGS 3DEP point queries. US-only.
...
Outside the US, or to check against what GeoFS itself renders, use the in-browser
...
`race/runways/` is being changed by another branch tonight, so check it before editing.)

**Steps:** a runway lives in **two** places that must match:
1. `race/runways/<id>.json` plus an entry in `race/runways/index.json`, using `touchdown.js`'s field
   names: `thr_lat`, `thr_lon`, `heading_deg`, `length_m`, `width_m`, plus `id`, `name`,
   `version`, `thr_alt_m` and `zone: {min_m, max_m}`.
2. The `RUNWAYS` dict in `race/server/app.py`, which is what the server actually scores against,
   because the deployed image ships `app.py`, not the runway files.

**Verify:** `test_runways_json_files_match_embedded_registry` in `race/test/test_server.py` passes.
...
```
This writes the `.glb` files and `race/models/index.json`. Models are nose along +X, up along +Y,
about 15 m long. If one looks rotated in-sim, fix its `offset.headingDeg/pitchDeg/rollDeg` in
`index.json`, not the mesh.

**Verify:** `cd race/test && pip install pygltflib numpy pytest && python -m pytest test_models.py -q`.
...
rebuild. The server re-reads it whenever a room draws its vote.
...
5. On success it writes `.deployed_sha` and logs `DEPLOY <sha> ok`. On failure with a healthy
   ...
   ok` (or `FAILED`, which needs a manual look). `.deployed_sha` is left alone on failure, so the
   next tick retries.

Flags other than `--dry-run` (e.g. `--allow-empty-db`) and env vars such as `RACE_ALLOW_EMPTY_DB=1`
pass straight through to `redeploy.sh`. The env overrides are listed in
...
**Steps**
```bash
race/server/redeploy.sh --dry-run   # print every command, run nothing
...
6. **4:** `docker build -f race/server/Dockerfile --build-arg GIT_SHA=<sha> -t race` from the
   checkout root.
7. **5:** replaces the container (`--user 99:100`, `--network proxy`, the data and courses mounts).
...
**Verify:** it prints `PASS`. Then run the [smoke test](#smoke-test-after-every-deploy). The poll
...
1. **DNS:** `race.finsonly.net` as an A record to the public IP, DNS-only, like the other
   subdomains.
2. **Copy the app** as a repo-shaped directory. The image builds from the repo root, and the server
   refuses to start with zero courses:
   ```bash
   ...
   ```
   Check it landed: `ls -la /mnt/user/appdata/stack/race-api/race/server/ /mnt/user/appdata/stack/race-api/race/courses/`.
...
4. **Compose (preferred):** merge [race/server/compose.snippet.yml](../race/server/compose.snippet.yml)
   ...
   `RACE_CHAT_RATE_PER_S` (defaults in [REFERENCE.md](REFERENCE.md#server)).
   ```bash
...
6. **Verify:** the [smoke test](#smoke-test-after-every-deploy). `docker logs race-api` should show
   `courses loaded: N from /app/courses`.
...
- **By hand** (safe with the container running). Never use `cp`: the database is in WAL mode, and a
  ...
  ```
- **Pruning:** no script in this repo deletes old backups, so they pile up in `DATA_DIR`. A
  `prune.sh` and a `redeploy.sh --no-prune` flag are *(unverified: not in the repo at the time of
  writing)*. Until then, delete old `race.db.bak-*` files by hand once you're sure a newer one is
  good.
...
**Restore** puts the snapshot back, and **Copy JSON** copies the results. Allowed aircraft writes in
`race.js` are only the ones the lab verified on 2026-09-23 (see CLAUDE.md).
...
*(Unverified, not recorded anywhere in the repo: git worktrees as the usual way to run parallel
sessions, and "Claude Code sessions on the PC can't push, so push from the VS Code terminal". If
that's the setup, add it here.)*
