...
- No writes to aircraft controls except `controls.setters.increaseThrottle`/`decreaseThrottle` (the green-flag throttle check and `GeoPhysics.airStart` only). `POWERUP_CONTROL_EFFECTS` stays `false`. Allowed physics writes are exactly the GeoFS calls verified in-sim on 2026-09-23 and 2026-09-24, and only through `GeoPhysics`: `geofs.flyTo([lat, lon, altM, hdg, true])` (air-start spawn, verified 2026-09-24), `geofs.aircraft.instance.place()` (teleport/fallback), `rigidBody.v_linearVelocity`/`setLinearVelocity()` (ENU m/s), `geofs.autopilot.setSpeed()`/`setAltitude()`/`setCourse()`/`turnOn()`/`turnOff()`, and `controls.setters.increaseThrottle`/`decreaseThrottle` (verified 2026-09-24). `geofs.resetFlight()`, direct `trueAirSpeed`/`groundSpeed` writes, and engine thrust multipliers are verified BROKEN — never reintroduce them.
