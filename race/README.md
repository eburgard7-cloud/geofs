...
race/
  ...
  courses/                shared courses (*.json), index.json (with cup/difficulty), CUPS.md
  models/                 joke-plane *.glb, index.json (id, file, scale, offsets), assignments.json (callsign → id)
  runways/                landing-mode runway defs; mirrored by RUNWAYS in server/app.py, which is what scores
  server/
    ...
    Dockerfile            builds from the REPO ROOT: bakes in app.py, static/, bookmarklet.txt, courses/
    redeploy.sh           one-command Unraid redeploy (see the runbook)
  ...
  tools/
    add_course.py         validate a pasted course JSON, write it and upsert courses/index.json
    check_terrain.py      sample terrain along a course (USGS 3DEP, Cesium ion, or a file)
    ...
    physics_lab.js        WRITE-capable test panel for GeoFS physics (LAB line; debug only)
...
count and settled rollout, into a 0–1000 score against a runway from `RUNWAYS`. It penalizes
