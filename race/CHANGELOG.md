...
- The fallback note no longer sits `position:absolute` over the 2D route/replay stage (it could
  ...
  "terrain is flat" degraded-3D note). The course page's `routeSvg` padding went 36 → 44 for extra
  headroom regardless.
...
- The route-change heading focus (`app.js`, for screen readers) no longer draws a visible focus
  ring — it was never a keyboard tab stop, so the big yellow outline was a false affordance.
...
- `site.js`: `probeCacheValid`, `classifyBlockReason`, `PROBE_FAILURE_TTL_MS` (pure, tested).
