...
| Tab 5d | Switch Pro, one lobby race, no screen touches after joining the room: **+** to ready, race with R/L/A, **+** to dismiss results | Ready toggles on +, race runs, + closes the results card | |
| Tab 5e | Hold **+** and **−** together ~2 s; in the panel press **Bind** for Slot 1 then press X; then Reset to defaults; then **+** | Panel opens after ~2 s (ring fills); ZL/ZR, D-pad, stick clicks, sticks shown as "GeoFS"; binding X for Slot 1 shows a "share button X" warning; reset clears it; + closes the panel without readying up | |
| Tab 6a | Tablet, in a lobby room then through one race: leave the tablet untouched on the gate for 5 min, then race | The screen never dims or locks while in the room or racing | |
